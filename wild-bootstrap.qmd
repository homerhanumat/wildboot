...
(**Note**:  In the above table, `holmbon` gives the value of $\alpha / (N-m+1)$ to which $p_m$ would be compared in the Holm-Bonferroni process.  We see that for the p-value 0.021 is less than the `holmbon` value, but it is not counted as significant because the Holm-bonferroni process stopped at the smaller p-value 0.02.)
