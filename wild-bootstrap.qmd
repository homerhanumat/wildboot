...
The data are shown in @tbl-motor-trend below:


```{r}
#| label: tbl-motor-trend
...
The bootstrap re-samples are given in a matrix (`mod$bootEstParam`) rows and columns are named, as represented in @tbl-bootsrap-resamples below:

```{r}
#| label: tbl-bootsrap-resamples
...
```{r}
#| label: tbl-param-point-estimates
...
```{r}
#| label: tbl-all-confints
#| tbl-cap: "95%-confidence intervals for parameters of the linear model"
#| code-fold: false
...
```{r}
#| label: tbl-p-vals
#| tbl-cap: "Parameter estimates with 95%-confidence intervals and p-values"
...
```{r}
#| label: tbl-holm-bon-hypo
#| tbl-cap: "Holm-Bonferroni applied to a few hypothetical p-values (alpha = 0.05)"
...
```{r}
#| label: tbl-holm-bon-example-data
#| tbl-cap: "Parameter estimates with 95%-confidence intervals, uncorrected p-values and Holm-Bonferroni rejection-criteria"
make_table <- function(mod, level = 0.95) {
  resamples <- mod$bootEstParam[, -1]
  parameters <- colnames(resamples)
  df <-
  ...
  df <-
    ...
    purrr::map_dfr(function(param) {
      ...
      list(
        ...
        lwr_bnd = interval[1],
        upr_bnd = interval[2],
        p_val = boot_p_val(model = mod, param = param, theta = 0)
      )
    })
  df
}
...
holm <- holm_bon(all_tabs$p_val)
all_tabs_complete <-
  all_tabs %>% 
  mutate(
      holmbon = holm$holmbon,
...
```{r}
#| label: tbl-all-tests
#| tbl-cap: "All tests, with confidence intervals, p-values, and Holm-Bonferroni assessment (alpha = 0.05)"
DT::datatable(
  all_tabs_complete %>% 
...
```{r}
#| label: tbl-all-tests-sig
#| tbl-cap: "All significant tests, with confidence intervals, p-values, and Holm-Bonferroni assessment (alpha = 0.05)"
knitr::kable(
  all_tabs_complete %>% 
    filter(significant) %>% 
    mutate(across(where(is.numeric), function(x) round(x, 5)))
...
### A Note on Adjusting P-values

R provices the function `p.adjust()` that offers to adjust P-values in accordance with several different approaches to multiple testing.  Here is an example of adjustments inspired by Holm-Bonferroni:

```{r}
#| code-fold: false
pvals <- c(0.02, 0.01, 0.021, 0.11)
p_holm <- p.adjust(pvals, method = "holm")
p_holm
```

Perhaps the clearest way to explain how the function arrives at the adjusted values is to reproduce it in R-code:

```{r}
#| code-fold: false
p_adjust_holm <- function(pvals) {
  ## find how many p-values there are:
  N <- length(pvals)
  ## find the order in which the values come:
  p_order <- order(pvals)
  ## sort the values:
  p_sorted <- sort(pvals)
  ## set up a "Results vector" to hold adjusted p-values
  p_adjusted <- numeric(N)
  ## now address the sorted p-values one by one:
  for (i in 1:N) {
    if (i == 1) {
      adjusted <- p_sorted[i] * (N - i + 1)
    } else {
      adjusted <- 
        max(
          p_sorted[i] * (N - i + 1),
          p[i - 1] ## assures that the adjusted values never decrease
        )
    }
    ## No P-value should exceed 1:
    if (adjusted > 1) adjusted <- 1
    ## fill in the results vector:
    p_adjusted[i] <- adjusted
  }
  ## permute the adjusted values so that their order corresponds to
  ## the order of the original p-values:
  p_adjusted[p_order]
}
```

Those adjusted values that do not exceed the target level of significance are the ones declared "significant":

```{r}
#| code-fold: false
#| label: tbl-p-adjust-example
#| tbl-cap: "Holm-Bonferroni adjusted p-values"
data.frame(
  p = pvals,
  p_adj = p.adjust(pvals, method = "holm")
) %>% 
  mutate(sig = p_adj <= 0.05) %>% 
  knitr::kable()
```

Readers might prefer to see the adjusted p-values:

```{r}
#| label: tbl-adjusted-study
#| tbl-cap: "Results showing unadjusted and Hom-Bonferroni-adjusted p-values"
DT::datatable(
  all_tabs %>% 
    mutate(p_adj = p.adjust(p_val, method = "holm")) %>% 
    mutate(across(where(is.numeric), function(x) round(x, 5))) %>% 
    select(dep, parameter, lwr_bnd, upr_bnd, p_val, p_adj)
)
```
