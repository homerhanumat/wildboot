...
I would note that the bootstrap is often used in hypothessis testing (i.e., there are ways to compute P-values).

Also, the need for the bootstrap comes soley from the failure of the data to meet all of the assumptions of the standard linear model (including normality of residuals):  it does not depend on number of independent or dependent variables.
...
confidence intervals.  If no all variables are independent of one another, then the chance of at least one "significant" result is:
...
*Using the bootstrap does nothing at all to address this issue.*
